...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Load UI file
        uic.loadUi('esp32_flash_simulator.ui', self)
    ...
    def _setup_canvas(self):
        """Setup the simulation canvas widget"""
        # Create canvas
        self.canvas = SimulationCanvas()
        
        # Replace the placeholder container with canvas
        layout = QVBoxLayout()
        layout.setContentsMargins(0, 0, 0, 0)
        layout.addWidget(self.canvas)
        self.canvasContainer.setLayout(layout)
    
    def _setup_connections(self):
        """Setup signal/slot connections"""
        self.executeBtn.clicked.connect(self._execute_operation)
        self.clearLogBtn.clicked.connect(self.logText.clear)
    
    def _initialize_log(self):
        """Initialize the log with startup message"""
        self.logText.append("ESP32-S3 Flash Simulator Ready")
        self.logText.append("=" * 60)
        self.logText.append("Flash Chip: Winbond W25Q256JWPIQ (256Mbit / 32MB)")
        self.logText.append("Interface: Quad SPI (QSPI)")
        self.logText.append("=" * 60)
    ...
    def _execute_operation(self):
        ...
        address = self.addressInput.value()
